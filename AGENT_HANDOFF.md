...
| Web UI (plan.md Workstream C) | **Done 2026-06-10** — React 19 + Vite + Tailwind v4 SPA in `webui/`, FastAPI server `localllm/webui/server.py` (`localllm-webui`, :8095) serving the static bundle + REST API (chat **+ agent mode with per-skill toggles** via `/api/skills`, **multi-file attachments** via `/api/uploads` attach/detach — images, audio, PDFs, docs reusing `media/attachments.py`; agent tool-call trace returned as `steps`; NDJSON-streaming audio translate, TTS, transcribe, OCR, health/meta). Mic capture via MediaRecorder with live level meter — foundation for Workstream B. Rebuild: `cd webui && npm install && npm run build` (Node is build-time only). `apps/webui.py` pre-checks the port and points at `scripts/stop_stale_ports.ps1` (now covers 8080/8090/8091/8095/8501/8502, verifies the kill, `-WhatIfOnly` dry run). Streamlit apps still alive. |
