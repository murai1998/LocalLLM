...
| Web UI (plan.md Workstream C) | **Done 2026-06-10** — React 19 + Vite + Tailwind v4 SPA in `webui/`, FastAPI server `localllm/webui/server.py` (`localllm-webui`, :8095) serving the static bundle + REST API (chat **+ agent mode with per-skill toggles** via `/api/skills`, **multi-file attachments** via `/api/uploads` attach/detach — images, audio, PDFs, docs reusing `media/attachments.py`; agent tool-call trace returned as `steps`; NDJSON-streaming audio translate, TTS, transcribe, OCR, health/meta). Mic capture via MediaRecorder with live level meter — foundation for Workstream B. Rebuild: `cd webui && npm install && npm run build` (Node is build-time only). `apps/webui.py` pre-checks the port and points at the platform's port-cleanup script — `scripts/stop_stale_ports.ps1` on Windows, `scripts/stop_stale_ports.sh` on macOS/Linux (both cover 8080/8090/8091/8095/8501/8502, verify the kill, support dry run). **`webui/dist` is committed** (`.gitignore` root-anchors `/dist/` so the global pattern no longer swallows it) — runtime needs no Node on a fresh machine; rebuild + recommit `webui/dist` after frontend changes. Streamlit apps still alive. |
| Live voice-to-voice (plan.md Workstream B) | **Done 2026-06-10** — `localllm/live/` (streaming endpointer + pipelined STT→MT→TTS asyncio session with rolling MT context, injectable stages), `/ws/translate` WebSocket in the web UI server (Int16 PCM in → JSON segment/transcript/translation/audio events out), gateway SSE pass-through for `stream: true`, browser Live mode (AudioWorklet capture, half-duplex gating, lag badges), `localllm-live-bench` harness (`--fake` = no GPU). Real-GPU validation pending: run `localllm-serve` then `localllm-live-bench file.wav --target en` (target: steady-state lag ≤ 8 s). |
| HF Spaces showcase (showcase-plan.md Plan A) | **S0 done 2026-06-11** — self-contained `showcase/` dir: Gradio app (live interpreter via FastRTC ReplyOnPause + @spaces.GPU Whisper-turbo STT + Gemma MT, Piper TTS on CPU; chat with image; file transcribe/translate; OCR ≤3 pages). Vendored prompts/voices/endpointer kept in sync by `scripts/build_showcase.py` + tests. Prominent "reduced-capability demo → full app on GitHub" notice in Space README and app banner. `SHOWCASE_PRESET=small` for local rehearsal. **User actions to publish:** PRO sub, Gemma license, create ZeroGPU Space, `HF_TOKEN` secret, `hf upload <user>/<space> ./showcase . --repo-type space`. Local-test deps: `pip install -e ".[dev,showcase]"` (tests skip without them). |
| Unit tests | **154 passed** on 3.14 / 150+4-skipped on 3.10 (client, gateway + guardrails + SSE, agent parser + tool security, media, TTS, secrets, web UI API, live endpointer/pipeline/WS, showcase sync + handler) |
...
**Immediate:** ~~smoke-test `localllm-serve` + `localllm-chat` on Mac Metal~~ **Done 2026-06-10** — gateway + web UI chat verified end-to-end on M4 Pro Metal (llama.cpp build 9590 via Homebrew; builds < 9590 fail with `unknown projector type: gemma4uv`). Next: Phase 3 profiles.
